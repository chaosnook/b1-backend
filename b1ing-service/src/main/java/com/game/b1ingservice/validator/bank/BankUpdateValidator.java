...
public class BankUpdateValidator extends CommonValidator {
    ...
    public void validate(Object o, Long id, Long agentId) {
...
//            throw new ErrorMessageException(Constants.ERROR.ERR_02006);
        ...
        if (bank.getBankGroup() != req.getBankGroup() && bank.getBankOrder() != req.getBankOrder()) {
            if (bankRepository.existsByBankOrderAndAgent_Id(req.getBankOrder(), agentId)
                    && bankRepository.existsByBankGroupAndAgent_Id(req.getBankGroup(), agentId)) {
